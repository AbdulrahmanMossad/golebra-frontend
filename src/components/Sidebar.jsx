...
const isNotActiveStyle =
  "flex items-center px-5 gap-3 text-gray-500 hover:text-black transition-all duration-200 ease-in-out capitalize hover:scale-105 isNotActive icon";
const isActiveStyle =
  "flex items-center  px-5 gap-3 font-extrabold  border-r-2 border-blue transition-all duration-200 ease-in-out capitalize hover:scale-105 isActive icon";
const Sidebar = ({ closeToggle, user }) => {
  ...
  return (
    <div className="flex flex-col justify-between h-full  bg-white overflow-y-scroll  hide-scrollbar home">
      <div className="flex flex-col home2">
        <Link to="/" onClick={handleCloseToggle} id="logo">
          <img
            src={logo}
            alt="logo"
            for="logo"
        ...
        <div className="flex flex-col gap-5">
          ...
          <h3 className="mt-2 px-5 text-base 2xl:text-xl icon">
            Discover cateogries
          </h3>
          ...
          <label className="themeSwitcherTwo shadow-card relative inline-flex cursor-pointer select-none items-center justify-center rounded-md icon p-1">
            <input
              type="checkbox"
              className="sr-only"
              checked={isChecked}
            ...
            {isChecked ? (
              <span
                className={`flex items-center space-x-[6px] rounded py-2 px-[18px] text-sm font-medium ${
                  isChecked
                    ? "flex my-5 mb-3 gap-2 p-2 items-center rounded-lg shadow-lg mx-3 sidebar"
                    : "text-unchecked"
                }`}
              >
                <svg
          ...
          {/* <a className="moon icon border " onClick={handelTheme}>
            {theme === "light" ? (
              <CiSun />
            ) : (
      ...
      {user ? (
        <Link
          ...
          className="flex p-3 gap-2 items-center shadow-lg  home2 "
          onClick={handleCloseToggle}
        >
          <img
            src={user.image}
            ...
            alt="user-profile"
          />
          <p className="icon">{user.userName}</p>
